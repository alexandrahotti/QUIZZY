...
function DATETIMEONLY() {
  if (!(this instanceof DATETIMEONLY)) return new DATETIMEONLY();
}
util.inherits(DATETIMEONLY, ABSTRACT);

DATETIMEONLY.prototype.key = DATETIMEONLY.key = 'DATETIMEONLY';
DATETIMEONLY.prototype.toSql = function() {
  return 'DATE';
};

DATETIMEONLY.prototype._stringify = function _stringify(date) {
  return moment(date).format('YYYY-MM-DD HH:mm');
};

DATETIMEONLY.prototype._sanitize = function _sanitize(value, options) {
  if ((!options || options && !options.raw) && !!value) {
...
DATETIMEONLY.prototype._isChanged = function _isChanged(value, originalValue) {
  if (originalValue && !!value && originalValue === value) {
    return false;
  }
 ...
 * @property {function} DATEONLY A date only column (no timestamp)
   @property {function} DATETIMEONLY A date only column (no timestamp)
